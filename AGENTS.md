...
- **Internal-only structs** (e.g. `MD_CONTAINER`) are plain `struct` (compiler-chosen layout). Keep `extern struct` ONLY where layout must mirror C: the `block_bytes` arena types (`MD_BLOCK`/`MD_LINE`/`MD_VERBATIMLINE`), `MD_MARK` (pointer-store trick), `MD_REF_DEF`/`MD_REF_DEF_LIST`, and `MD_PARSER` (a real C-shaped callback table until Phase 4c step 3).
- **SAX detail types** (`MD_ATTRIBUTE`, `MD_BLOCK_*_DETAIL`, `MD_SPAN_*_DETAIL` in `src/abi.zig`) are plain `struct`s with **slices** (`text`, `meta`, `highlights`, `raw_attrs`, `raw_props`, `title`, `substr_types`, `substr_offsets`) and `bool` for the two-state flags (`is_tight`, `is_task`, `is_autolink`). Slice lengths are **exact**: `substr_types.len == substr_count`, `substr_offsets.len == substr_count + 1`, and `MD_ATTRIBUTE.size()` is `text.len`. An absent value is the **empty slice** — do not reintroduce a nullable pointer or a parallel `*_size`/`*_count` field, and do not treat empty as meaningfully different from absent. Walk `MD_ATTRIBUTE` with a bounded loop (`i < attr.substr_types.len and attr.substr_offsets[i] < attr.size()`), never a bare terminator walk. Because these are auto-layout structs, `MD_SPAN_A_DETAIL` and `MD_SPAN_IMG_DETAIL` are **no longer layout-compatible** and a detail scratch slot cannot be an `extern union` — `process.zig` uses a struct plus an explicit per-block-type selector (`detailPtr`) instead.
- **`export` / `callconv(.c)` only at real boundaries:** the internal entry points (`md_parse`, `md_html`/`md_html_ex`, `md_ast`, `md_ansi`, `md_text`, `md_markdown`, `md_meta`, `md_heal`, `entity_lookup`) and the `process_output` sink are **plain Zig** — no `export`, no `callconv(.c)`. Do not add either back. They survive in exactly four places, all genuine boundaries: the **wasm exports** (`md4x_to_html`, `md4x_alloc`, …), the **napi** module registration and its registered callbacks, the **`MD_PARSER` SAX callbacks** (Phase 4c retires those), and the **`qsort`/`bsearch` comparators** in `refdefs.zig` that are handed to libc (constraint: glibc tie-break parity).
