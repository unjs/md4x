...
| `{expr}` in running text                            | inline syntax and escaping live inside                   | verbatim expression run where it cannot be an attribute list (`spec-binding.txt`) | deliberate — JSX-style `{a*b}`, `{a < b}` reach a post-processor as written |
