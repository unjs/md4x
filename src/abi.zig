...
pub const BlockLiDetail = struct {
    /// True when the item opens with a `[ ]` / `[x]` task marker.
    is_task: bool = false,
    /// 'x', 'X' or ' ' when `is_task`.
    task_mark: MD_CHAR = 0,
...
/// The SAX callback table. A plain Zig struct: no `extern`, no
...
// There is no parser flag word. md4x has exactly ONE dialect, so every
// extension the parser supports (tables, strikethrough, task lists, LaTeX math,
// frontmatter, components, attributes, alerts, highlight, footnotes and the
// three permissive-autolink kinds) is unconditionally on. The `MD_FLAG_*` /
// `MD_DIALECT_*` constants and `Parser.flags` are gone — do not reintroduce
// them, and port upstream flag-gated code as unconditional code.
